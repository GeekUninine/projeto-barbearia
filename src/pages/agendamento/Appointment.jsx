...
function Agendamento() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://projeto-barbearia-production.up.railway.app/agendar",
        {
          method: "POST",
  ...
  const renderHorarios = () => {
    ...
    const dataAtual = `${horaAtual.getFullYear()}-${String(horaAtual.getMonth() + 1).padStart(2, "0")}-${String(horaAtual.getDate()).padStart(2, "0")}`;
    const isHoje = data === dataAtual;
  
    return horarios
      .filter((hora) => !isHoje || hora > horaAtualFormatada)
      .map((hora) => (
